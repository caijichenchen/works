  ...
  if (message.MsgType === 'text') {
    ...
    if (message.Content === '热门') {  //全匹配
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        content.push({
          title: item.title,//标题
          description: item.summary,//简介
          picUrl: `http://peicjnx2h.bkt.clouddn.com/${item.posterKey}`,//图片
          url: `${url}/detail/${item.doubanId}`//详情链接
        })
      }
    ...
    if (message.Event === 'subscribe') {
      //用户订阅事件
      content = '欢迎您关注谷粒电影公众号~ \n' +
      '回复 首页 查看谷粒电影预告片 \n' +
      '回复 热门 查看最热门的电影 \n' +
      '回复 文本 搜索电影信息 \n' +
      '回复 语音 搜索电影信息 \n' +
      '也可以点击下面菜单按钮，来了解谷粒电影公众号';
